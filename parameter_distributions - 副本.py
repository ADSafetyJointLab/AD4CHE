...
def plot_rel_vx0(overlap_data, nonoverlap_data):
    ...
    ax[1].set_xlabel(r'Initial relative longitudinal velocity v$_{\rm rel,0}$ [m/s]')
...
def plot_max_vy(overlap_data, nonoverlap_data):
    ...
    ax[0].set_ylim([0, 2.5])
    ax[1].set_ylim([0, 2.5])
    ax[1].set_xlabel(r'Maximum lateral velocity of the challenging vehicle v$_{y,\rm max}$ [m/s]')
    ax[0].set_ylabel('Density')
    ax[1].set_ylabel('Density')
    ax[0].grid(alpha=0.75)
    ax[1].grid(alpha=0.75)
...
def plot_max_offset(overlap_data, nonoverlap_data):
    ...
    # ax[0].set_ylim([0, 2.5])
    # ax[1].set_ylim([0, 2.5])
    ax[1].set_xlabel(r'Lateral offset of the ego vehicle when crossing lane marking [m]')
    ax[0].set_ylabel('Density')
    ax[1].set_ylabel('Density')
    ax[0].grid(alpha=0.75)
    ax[1].grid(alpha=0.75)
...
class ParameterDistribution:
    ...
    def get_data(self, car_pairs):
        ...
        for key, car_pair in car_pairs.items():
            ...
            ego = car_pair[1]
            key_timestamps_t3 = car_pair[2][1]
            ...
            for car in cutin_car:
                if car[self.frame] == key_timestamps_t3:
                    y_offset = abs(float(car[self.ego_offset]))
                    offset_max.append(y_offset)
                    break

        return rel_dx0, rel_vx0, ve0, vy_max, offset_max


def main():
    obj = utils.load_object("data.pickle")
    para_data = ParameterDistribution(obj.frame, obj.width, obj.vx, obj.vy, obj.x, obj.ego_offset)
    overlap_rel_dx0, overlap_rel_vx0, overlap_ve0, overlap_vy_max, overlap_offset_max = para_data.get_data(
        obj.overlap_car_pairs)
    nonoverlap_rel_dx0, nonoverlap_rel_vx0, nonoverlap_ve0, nonoverlap_vy_max, nonoverlap_offset_max = para_data.get_data(
