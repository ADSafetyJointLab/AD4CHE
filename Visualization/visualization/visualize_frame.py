            ...
            if initial_frame <= self.current_frame < final_frame:
                ...
                try:
                    ...
                    if self.arguments["background_image"] is not None:
                        bounding_box /= meterPerPixel
                        # bounding_box /= 4
                        bounding_box[0] = bounding_box[0] - bounding_box[2]/2
                        bounding_box[1] = bounding_box[1] - bounding_box[3]/2
                ...
                if self.arguments["plotDirectionTriangle"]:
                    ...
                    pointx = bounding_box[0] + bounding_box[2] * 0.5
                    pointy = y_position + (bounding_box[3] / 2)
                    if current_velocity < 0:#上面车辆为负
                        ...
                        triangle_info = np.array([[x_back_position, x_back_position, bounding_box[0] + bounding_box[2]],
                                                  triangle_y_position])
                    for i in range(3):
                        triangle_info[0][i], triangle_info[1][i] = Nrotate(track[ORIENT][current_index], triangle_info[0][i], triangle_info[1][i], pointx, pointy)
                ...
                if self.arguments["plotTrackingLines"]:
                    ...
                    if relevant_bounding_boxes.shape[0] > 0:
                        if self.arguments["background_image"] is not None:
                            relevant_bounding_boxes /= meterPerPixel
                        ...
                        x_centroid_position = relevant_bounding_boxes[:, 0]
                        y_centroid_position = sign * relevant_bounding_boxes[:, 1]
